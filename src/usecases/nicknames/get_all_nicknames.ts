...
export async function getAllNicknames(setterId: string): Promise<GetAllNicknamesResult> {
  ...
  if (!normalizedSetterId) {
    throw new AppError<GetAllNicknamesErrorType>("MISSING_INPUT", "User ID is required.", 400);
  }

  if (!isValidUuid(normalizedSetterId)) {
    throw new AppError<GetAllNicknamesErrorType>("INVALID_FORMAT", "Invalid user ID format.", 400);
  }
  ...
  try {
    ...
    if (isPgErrorCode(error, PgErrorCode.INVALID_TEXT_REPRESENTATION)) {
      throw new AppError<GetAllNicknamesErrorType>("INVALID_FORMAT", "Invalid ID format.", 400);
    }

    console.error(`[ERROR] Unexpected error in use case: Get all nicknames\n`, error);
    throw new AppError<GetAllNicknamesErrorType>("INTERNAL_ERROR", "Internal server error.", 500);
  }
}
